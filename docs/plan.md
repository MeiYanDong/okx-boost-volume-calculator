...
S26 将“发送测试”从纯连通性消息改为真实数据演练。服务端读取当前登录用户的 Supabase 归档，执行一次每日自动刷新演练，生成真实预测摘要后发送到该用户自己的飞书机器人；测试不写入归档。

子 plan：[S26 飞书真实数据测试发送](./plans/2026-05-10-real-data-feishu-test.md)
