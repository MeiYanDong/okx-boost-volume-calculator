...
S25：取消全局飞书 Webhook 已完成。

S25 明确飞书 Webhook 是用户个人通知通道。`/api/feishu` 必须登录后使用个人配置，Cron 未找到工作区所属用户个人 Webhook 时不发送通知，测试消息也不包含真实钱包或交易数据。

子 plan：[S25 取消全局飞书 Webhook](./plans/2026-05-10-personal-feishu-only.md)
