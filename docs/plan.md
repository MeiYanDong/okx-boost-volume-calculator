...
S29 继续从多用户隔离和计算口径审查。已修复服务 API 优先使用私有访问码导致登录账号状态被绕过的问题，并将首页、钱包列表和统计报表统一改为按当前快照日 10 日窗口动态聚合；生产部署和冒烟验证已完成。

子 plan：[S29 会话与快照窗口审查](./plans/2026-05-10-session-window-audit.md)
