...
S24 将飞书提醒从全局环境变量升级为账号级配置。每个登录用户可以在偏好设置里保存自己的飞书机器人，手动风险提醒和每日 Cron 都优先使用该用户配置，未配置时保留全局 Webhook 兜底。已完成 Supabase 迁移、真实 API 测试、生产部署和线上 smoke test。

子 plan：[S24 用户级飞书通知配置](./plans/2026-05-10-user-feishu-settings.md)
...
| S23 管理员用户管理 | 已完成 | [子 plan](./plans/2026-05-10-admin-user-management.md) | 管理员查看用户、调整钱包额度、启用或禁用账号 |
