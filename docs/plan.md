...
S41 修复“候选交易 hash 已发现，但逐笔解析失败后仍保存半成品”的问题。RPC 临时错误会重试；任意候选 hash 未解析完成时，本次扫描失败并保留旧归档；Supabase 写入前会拒绝交易明细不完整的结果，同时允许完整新结果覆盖历史残缺归档。

子 plan：[S41 交易明细完整性加固](./plans/2026-05-13-transaction-detail-completeness.md)
