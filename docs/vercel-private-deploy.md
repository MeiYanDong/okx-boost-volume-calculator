...
14. 做 `/api/cron/daily-refresh?dryRun=1` 验证，确认不会真实发送飞书。
15. 如果配置了飞书 Webhook，只在确认要测试真实通知时再触发非 dry-run 请求。
...
12. Vercel Cron 配置为 `5 0 * * *`，对应北京时间 08:05。
13. Cron 请求必须通过 `CRON_SECRET` 或私有访问码验证。
