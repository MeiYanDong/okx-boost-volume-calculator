...
5. Supabase 保存邀请制账号、钱包归档和扫描结果，避免重新部署后数据丢失。
...
| `SUPABASE_PUBLISHABLE_KEY` | Supabase publishable key，用于登录校验 | 多用户部署必须配置 |
| `SUPABASE_SECRET_KEY` | Supabase secret/service key，只能放服务端 | 多用户部署必须配置 |
...
16. 如果配置了飞书 Webhook，只在确认要测试真实通知时再触发非 dry-run 请求。
...
8. 如果配置了飞书 Webhook，页面只在快照预测存在风险时显示飞书提醒按钮。
