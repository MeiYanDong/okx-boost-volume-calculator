...
4. 增量刷新时，如果失败链已有同窗口原归档，沿用该链归档并记录 warning。
5. 其他链可以继续刷新，最终结果仍包含失败链的原归档，不会保存缺链半成品。
6. 强制重扫不复用旧归档，仍要求完整重新发现交易。

## 验收

1. X Layer Ankr 索引不可用时，错误信息展示真实索引原因。
2. 同窗口已有 X Layer 归档时，增量刷新不再因为 X Layer 临时索引失败阻断 BNB Chain 更新。
